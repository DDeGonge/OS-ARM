...
void motorDrive::set_pos_target_mm_async(double target, float feedrate)
{
    ...
    // Set direction if stopped already. Otherwise it's handled in the stepchecker
    if (abs(current_velocity) < 0.001)
    {
        // Serial.println("CHANGING DIR STOPPED");
    ...
    // Update set velocity if updates are sent frequently enough, makes motion smoother
    double command_vel = 0.2 * 1000000 * abs(target_mm - last_target_mm);
    command_vel /= (next_step_us - last_update_us);
    if (feedrate != NOVALUE)
        set_vel = feedrate;
    else if ((next_step_us - last_update_us) < 1000000)
        set_vel = min(max_vel, command_vel);

    // Serial.print(current_velocity);
    // Serial.print("\t");
    // Serial.println(set_vel);
    last_target_mm = target_mm;
    last_update_us = next_step_us;
}
...
bool motorDrive::step_if_needed(uint32_t t_now)
{
    ...
    if(t_now > next_step_us)
    {
        ...
        // Serial.print("\t");
        // Serial.println(set_vel);
        if((stepper.get_dir() && ((stop_pos_mm + stop_dist_mm) > target_mm)) || (!stepper.get_dir() && ((stop_pos_mm - stop_dist_mm) < target_mm)) || abs(current_velocity) > set_vel)
        {
            // First check if we are coming to a stop
            if((pow(current_velocity, 2) - 2 * max_accel * step_size_mm) < 0)
            {
                // Serial.println("CHANGING DIR IMP");
                // See if we should turn round
                if(stepper.get_step() > step_target)
                {
                    next_step_us = (2 * next_step_us) - last_step_us;
                    diff_exact_us = next_step_us - last_step_us;
                    stepper.set_dir(false);
                }
                else if(stepper.get_step() < step_target)
                {
            ...
            else
            {
                ...
                diff_exact_us = 1000000 * min(t0, t1);
                next_step_us = (uint32_t) (diff_exact_us + step_error_us);
                step_error_us = diff_exact_us - (double) next_step_us;
                next_step_us += cur_step_us;
            }
        }

        // Otherwise check if we can speed up
        else if(abs(current_velocity) < set_vel)
        {
            // Quadratic has 2 roots, only use one that results in positive time
            double t0, t1;
            quad_solve(t0, t1, max_accel, current_velocity, step_size_mm);
            diff_exact_us = 1000000 * max(t0, t1);
            next_step_us = (uint32_t) (diff_exact_us + step_error_us);
            step_error_us = diff_exact_us - (double) next_step_us;
            next_step_us += cur_step_us;
        }
        ...
        {
            diff_exact_us = step_size_mm;
            diff_exact_us *= (1000000.0 / set_vel);
            next_step_us = (uint32_t) (diff_exact_us + step_error_us);
            step_error_us = diff_exact_us - (double) next_step_us;
            next_step_us += cur_step_us;
        }
        ...
        current_velocity = abs(current_velocity) < 0.001 ? 0 : current_velocity;
...
void motorDrive::plan_move(double target, float feedrate, float accel, bool ignore_limits)
{
    ...
    float maxvel = feedrate == NOVALUE ? set_vel : feedrate / 60;
    ...
    Serial.print("spos: ");
    Serial.print(stepper.get_step() / steps_per_mm);
    Serial.print("\ttar: ");
